...
function Write-Theme {
    ...
    #check for elevated prompt
    If (Test-Administrator) {
        Write-Prompt -Object "$($SL.PromptSymbols.ElevatedSymbol) " -ForegroundColor $([System.ConsoleColor]::Yellow) -BackgroundColor $LastBackgroundColor
    }
    ...
    if ($status) {
        $themeInfo = Get-VcsInfo -status ($status)
        $TempArray = $themeInfo.VcInfo -split " "
        $TempArray[1] = Compress-String $TempArray[1]
        $themeInfo.VcInfo = $TempArray -join " "
